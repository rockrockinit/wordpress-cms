...
class Record {
	...
	public function __call( $name, $args ) {
	  if (preg_match('/^get_/', $name) && method_exists($this, $name)){
	    if ($args) {
	 ...
	 * @return \WordPress\CMS\DB\Column The column.
	 ...
	 * @return boolean|\WordPress\CMS\DB\Record
	 */
	 ...
	 *
	 * @param string|\WordPress\CMS\DB\Table  $foreign_table The foreign table.
	 * @param string|\WordPress\CMS\DB\Column $foreign_column The column in the foreign table that references this record's table.
  ...
  /**
   * Get records for a column of type ids
   *
   * @param $column
   * @return array
   */
	public function get_records ($column)
  {
    $records = [];

    if (preg_match('/_ids$/i', $column) && isset($this->data->{$column}) && $this->data->{$column}) {
      $database = $this->table->get_database();

      $table = preg_replace('/_ids$/i', '', $column);
      $table = new Table( $database, $table );

      $ids = Utils::trimExplode('/,|\|/', $this->data->{$column});
      if ($ids) {
        $sql = "SELECT * FROM {$table->get_name()} WHERE id IN ('" . implode("','", $ids) . "')";
  ...
  /**
   * Get a columns of this record's table
   *
   * @param string  $name The name of the column.
   * @param boolean $required True if this should throw an Exception.
   * @return \WordPress\CMS\DB\Column The column.
   * @throws \Exception If the column named doesn't exist.
   */
  protected function get_columns($column = '') {
    $columns = [];
