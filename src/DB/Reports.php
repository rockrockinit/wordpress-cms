...
class Reports {
	 ...
	 *
	 * @param \WordPress\CMS\DB\Database $db The database object.
	 */
	...
	public static function report_sources_table_name() {
		global $wpdb;
		return $wpdb->prefix . CMS_SLUG . '_report_sources';
	}
	...
	public function get_template( $report_id ) {
		...
		if ( false === $reports ) {
			$msg = "Reports table not found. Please re-activate Cms to create it.";
		...
		$template = new \WordPress\CMS\Template( false, $report->template() );
	...
	public static function activate() {
		...
		if ( '0' === $wpdb->get_var( "SELECT COUNT(*) FROM `" . self::reports_table_name() . "`" ) ) {
			...
			. "  <dt><a href='{{ admin_url('admin.php?page=cms&controller=reports&id='~report.id) }}'>{{report.title}}</a></dt>\n"
