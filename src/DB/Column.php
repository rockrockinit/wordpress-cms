...
class Column {
	 ...
	 *
	 * @param \WordPress\CMS\DB\Table $table The table that this column belongs to.
	...
	public function get_title() {
	  $name = preg_replace('/_ids$/i', '', $this->get_name());

		return \WordPress\CMS\Text::titlecase( $name );
	}
	...
	public function get_current_column_definition() {
	  ...
	  // Handles Enums & Sets
	  if (preg_match('/^(enum|set)$/i', $this->get_xtype()['name'])) {
      $size = implode(',', $this->get_options());
    }
	 ...
	 * @param \WordPress\CMS\DB\Table $target_table The target table for a foreign key.
    ...
    // Handle Enums & Sets
    if (preg_match('/^(set|enum)$/i', $xtype['name'])) {
		  $values = Utils::trimExplode(',', $size);

      if ($values) {
        $size_str = '(\'' . implode("','", $values) . '\')';
      }
    }
		...
		$ref_str = '';
		$sign_str = '';
		if ( $target_table instanceof \WordPress\CMS\DB\Table ) {
