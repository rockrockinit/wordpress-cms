...
class RecordController extends ControllerBase {
	 ...
	 *
	 * @param \WordPress\CMS\DB\Table $table The table.
	...
	public function save( $args ) {
	  //die('<pre>' . print_r($args, 1) . '</pre>');
		...
		$this->verify_nonce( 'cms-record-' . $record_ident );
		$template = $this->get_template( $table );
		...
		if ( ! $record_ident && $existing ) {
			...
			try {
				...
				// Prepares values for set datatypes
        ...
        //die('<pre>' . print_r($data, 1) . '</pre>');
				...
				$template->record = new \WordPress\CMS\DB\Record( $table, $data );
			}
		}
