...
class MapController extends ControllerBase {
	...
	/**
	 * Get the Cms 'by line' for inclusion in exports.
	 *
	 * @return string
	 */
	protected function byline() {
		return 'Cms ' . CMS_VERSION . ' (WordPress plugin)';
	}
	...
	public function osm( $args ) {
		...
		foreach ( $this->table->get_records( false ) as $record ) {
			...
			foreach ( $this->table->get_columns() as $col ) {
				...
				$fktitle = $col_name . \WordPress\CMS\DB\Record::FKTITLE;
	...
	public function gpx( $args ) {
		...
		foreach ( $this->table->get_records( false ) as $record ) {
			...
			foreach ( $this->table->get_columns() as $col ) {
				...
				$fktitle = $col->get_name() . \WordPress\CMS\DB\Record::FKTITLE;
